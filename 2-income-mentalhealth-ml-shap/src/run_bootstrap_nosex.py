# -*- coding: utf-8 -*-
"""
Bootstrap Tables — sex excluded from features.
Identical to v2 except 'sex' is dropped from the feature matrix
before any feature selection or model training.
Results saved to results/median_nosex/ and results/q25_nosex/
"""
...
# =============================================================================
# 1. Model build + tune (identical to v2)
# =============================================================================
...
# =============================================================================
# 2. Table formatting (identical to v2)
# =============================================================================
...
def latex_table2(results_income, results_mental):
    ...
    return (
        "\\begin{table}[ht]\n"
        "\\caption{Performance Comparison of ML Models (sex excluded from features)}\n"
        "\\label{table_ML_1}\n\\centering\n"
        "\\begin{tabular}{llccc}\n\\toprule\n"
        ...
        "Sex excluded from the feature set. "
        "Means over 50 bootstrap iterations. "
        "Within-country median cutoff. "
...
def latex_table3(res_f_inc, res_m_inc, res_f_mnt, res_m_mnt):
    ...
    for task_label, res_f, res_m in [
        ...
        for sg, res, best in [("Female", res_f, best_f), ("Male", res_m, best_m)]:
            for i, (m, label) in enumerate(model_order):
                auc = np.mean(res[m]['auc'])
                f1  = np.mean(res[m]['f1'])
                acc = np.mean(res[m]['acc'])
    ...
    return (
        "\\begin{table}[ht]\n"
        "\\caption{Heterogeneity Analyses by Sex (sex excluded from features)}\n"
        "\\label{table_ML_2}\n\\centering\n"
...
# =============================================================================
# 3. Data preparation — SEX EXCLUDED
# =============================================================================
...
def prepare_datasets(quantile_upper):
    ...
    # ── KEY CHANGE: exclude sex from feature matrix ──────────────────────────
    base_ml = base_ml.drop(columns=['sex'], errors='ignore')
    print(f"  [nosex] Feature columns after sex removal: {len(base_ml.columns)-1} features "
          f"(childid excluded)")
...
# =============================================================================
# 7. Run — median only first
# =============================================================================
run_variant(0.5, 'median_nosex')
