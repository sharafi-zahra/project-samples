# -*- coding: utf-8 -*-
"""
Bootstrap Tables v3 — country dummies added as features.

Changes vs v2:
  1. Country dummies (ET reference, IN/PE/VN indicators) added to feature matrix
  2. Within-country threshold and standardization unchanged
  3. Results saved to median_v3 / q25_v3
"""
...
def prepare_datasets(quantile_upper):
    ...
    country_dummies = pd.get_dummies(base['country'], prefix='country', drop_first=True)
    base_ml = base.drop(columns=['country'], errors='ignore')
    base_ml = pd.concat([base_ml.reset_index(drop=True),
...
def run_variant(quantile_upper, label):
    ...
    outdir = os.path.join('results', label + '_v3')
    ...
    print(f"\nVariant '{label}' complete. Results in: results/{label}_v3/")
